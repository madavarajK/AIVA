...
class TextSpeech:
    ...
    def opentxt(self):
        ...
        try:
            file_path=filedialog.askopenfilename(initialdir =r"C:\Users\Madavaraj K\OneDrive\Documents\Notes",title="Select file",filetypes=(('text file',"*.txt"),("All files", "*.*")))
...
class screenshot:
    def takeSS(self):
        ...
        os.chdir(a)
class ipaddress:
    def ipadd(scrallable_text):
        SR=SpeakRecog(scrallable_text)
        ip=requests.get('https://api64.ipify.org?format=json').json()
        SR.speak("yout IP Address is.",ip)
...
class Weather:
    def show(self,scrollable_text):
        SR=SpeakRecog(scrollable_text)
        base_url = "https://api.openweathermap.org/data/2.5/weather?q=bangalore,IN&units=metric&appid=49e823605c74297c2e2f1b5b6fe597f5"
        data=requests.get(base_url).json()
        SR.scrollable_text_clearing()
        SR.speak("-------------------------Weather Report of Bangalore City------------------------------")
        SR.speak("Temperature:   "+str(int(data['main']['temp']))+' Celsius\n'+
